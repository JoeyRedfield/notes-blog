---
tags: [index]
---

# AI 编码实践

AI 编码工作流、方法论与实战记录。

> [!note]
> 这个目录主要以 `[社区快照]` 和 `[实验观察]` 为主：
>
> - 演讲、工作坊、方法论总结 → `[社区快照]`
> - 你自己的 PRD、实战记录、能力路线、关系图验证 → `[实验观察]`
>
> 这里默认不是“官方产品机制区”，而是：
>
> - 学别人怎么做
> - 再看你自己怎么试过
> - 最后沉淀成能复用的方法

## 内容索引

- [[Codex 沙盒与权限设置]] — `[官方机制]` 整理 Codex 沙盒、批准策略、对话框权限 preset 与当前会话覆盖默认配置之间的关系
- [[Codex Superpowers 插件与 brainstorming skill 的关系]] — `[实验观察]` 解释 Codex `superpowers` 插件、`brainstorming` skill 与当前会话技能暴露层之间的真实关系
- [[AI编码全流程工作流——Matt Pocock 工作坊]] — `[社区快照]` Matt Pocock 的完整 AI 编码六阶段流程
- [[软件基础在AI时代更重要——Matt Pocock]] — `[社区快照]` AI 时代软件基础更重要、五大失败模式
- [[软件基础与Agent工程——Uncle Bob谈AI时代的软件开发]] — `[社区快照]` Uncle Bob 论确定性质量门、模块设计、多 Agent 流水线与战略编程
- [[01-生成式软件工程开篇——欢迎来到未来（NJU）]] — `[社区快照]` 南京大学生成式软件工程第一课：Agent 能力飞轮、意图—规约—实现鸿沟与可维护架构
- [[提示词工程——从 Prompt 到 Context Engineering（NJU）]] — `[社区快照]` 南京大学生成式软件工程第二课：完整上下文、verifier、注意力污染与 long-horizon Agent
- [[AI编码能力提升路线]] — `[实验观察]` 基于当前技术背景梳理的六大 AI 编码提升方向
- [[三角循环——AI编码核心概念内化记录]] — `[实验观察]` 交互式 Review 的心智模型内化记录
- [[inbound-order-PRD]] — `[实验观察]` 入库单管理系统 PRD
- [[inbound-order-实战记录]] — `[实验观察]` AI 编码全流程实战记录

## 相关笔记

- [[Agent与自动化/README|Agent 与自动化]] — Agent 工程化与自动化方案
- [[Claude Code/README|Claude Code]] — Claude Code CLI 配置与技巧
