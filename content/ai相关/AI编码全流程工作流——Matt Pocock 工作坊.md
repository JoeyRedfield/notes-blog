...
**Kanban 看板 + DAG 拆分**：在格力做 WMS/MES 开发时，把功能需求拆成有依赖关系的小 issues（垂直切片），标注哪些可以独立并行、哪些有依赖——这跟制造业软件的分层架构天然契合。

**深度模块实践**：Spring Boot Service 层天然是深度模块——设计好 Service 接口和 Repository 边界，Service 内部逻辑可以更放心交给 AI。

**通用语言文件**：为 WMS/WCS 建立一份术语定义文件（入库/上架/拣货/波次/库位...），在 AI 协作时始终传入——这是从 Matt 的 Ubiquitous Language Skill 直接借鉴的做法。
